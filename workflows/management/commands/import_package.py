...
def import_package(package_name, writer, external=False):
    ...
    if external:
        ...
        package_directory = os.path.join(os.path.dirname(os.path.realpath(__file__)),
    ...
    if not os.path.exists(package_directory) or not os.path.exists(widgets_directory):
    ...
    if os.path.exists(categories_directory):
        ...
        writer.write(' > No categories exported\n')
    ...
    for widget_file in widget_files:
        ...
        if w_data != serialize_widget(aw):
            ...
            for inp in inputs:
                ...
                i.save()
            # find stale inputs
            stale_ais = AbstractInput.objects.filter(widget=aw).exclude(
                uid__in=[inp['fields']['uid'] for inp in inputs])
            if stale_ais:
                stale_ais.delete()
            ...
            for out in outputs:
                ...
                o.widget = aw
                o.save()
            stale_aos = AbstractOutput.objects.filter(widget=aw).exclude(
                uid__in=[out['fields']['uid'] for out in outputs])
            if stale_aos:
                stale_aos.delete()
            ...
            for option in options:
                ...
                o.save()
            stale_os = AbstractOption.objects.filter(abstract_input__widget=aw).exclude(
                uid__in=[o['fields']['uid'] for o in options])
            if stale_os:
                stale_os.delete()
        ...
        for deprecated_widget_file in deprecated_widgets_files:
            ...
            try:
                ...
                if Widget.objects.filter(abstract_widget=aw).count() == 0:
                    ...
                    writer.write(style.ERROR('   - The widget ' + str(widget['fields'][
                                                                          'name']) + ' is still used in workflows. It was not removed, but it is deprecated!\n'))
            except AbstractWidget.DoesNotExist:
                pass
