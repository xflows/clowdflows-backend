            ...
            if not USE_CONCURRENCY or 1 == 1:
                ...
                try:
                    while len(unfinished_list) > 0:
                        ...
                        self.widget.progress = (int)((current_iteration * 100.0 / progress_total) + (
                            ((completed * 1.0) / total) * (100 / progress_total)))
                        self.widget.save()
                        unfinished_list = self.get_runnable_widgets()
                except:
                    raise
...
class Widget(models.Model):
    ...
    def reset_descendants_slow(self):
        ...
        widgets = list(
            self.workflow.widgets.prefetch_related('inputs', 'outputs', 'inputs__connections', 'outputs__connections',
                                                   'outputs__connections__input', 'inputs__connections__output'))
        widgets_dict = {}
        widgets_that_need_reset = set([self.pk, ])
        current_widgets_that_need_reset = set([self.pk, ])
        for w in widgets:
